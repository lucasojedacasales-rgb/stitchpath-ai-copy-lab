# REFERENCE_INTEGRATED_PIPELINE_AFTER_SATIN_V2 — StitchPath AI
...
La medición integrada ya queda instrumentada en `prof.report.integratedSatinValidation` dentro de `applyProfessionalPipeline`.
...
## Criterio safeToKeepSatin=true

`safeToKeepSatin` se calcula en runtime como true solo si:

- integratedValidation=true
- satinContourCount sube
- runningContourCount baja o se mantiene
- visibleDiagonalStitches no sube respecto al estado antes de SATIN
...
SATIN_PHASE_ORDER_FIX_V1 deja la validación integrada activa en el orden nuevo. La decisión final `safeToKeepSatin` ya no depende de un baseline aislado: se calcula con los comandos devueltos por Trim Guard + Splitter V1_2 justo antes de SATIN y con los comandos devueltos por SATIN justo antes del quality gate final.