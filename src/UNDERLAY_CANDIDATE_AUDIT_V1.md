# UNDERLAY_CANDIDATE_AUDIT_V1 — StitchPath AI
...
Con esos límites, los candidatos fuertes serían:
...
### Por qué centerline es mejor para CE01

CE01 necesita evitar:
...
- diagonales visibles,
- micro-puntadas,
- crecimiento brusco de stitchCount.

Un centerline underlay limitado a 1–2 pasadas cortas por región grande añade pocas puntadas y puede insertarse dentro del mismo bloque de color/relleno, reduciendo saltos nuevos.
...
### Ubicación recomendada para underlay

`UNDERLAY_GENERATOR_V1` debería ejecutarse:

**después de `reorderProfessionalLayers`, antes de visible diagonal repair/travel sanitize y antes de Trim Guard.**
...
No se recomienda ejecutarlo después de Trim Guard, porque cualquier salto o trim nuevo quedaría fuera del guard que ya está estabilizado.

---

## 5. Límites de seguridad propuestos

Valores iniciales recomendados para V1:
...
### Cómo evitar diagonales visibles

- Solo crear segmentos internos con longitud <= 4.0mm.
- No conectar regiones entre sí con stitches; usar jump/trim existente solo si ya está dentro del bloque.
...
### Cómo evitar saltos/trims excesivos
...
Revertir la fase completa si ocurre cualquiera de estos casos:
...
La futura implementación debe generar:

`UNDERLAY_GENERATOR_REPORT_V1.md`
...
| `stitchCountBefore` / `stitchCountAfter` | Conteo global. |
| `jumpCountBefore` / `jumpCountAfter` | Conteo global. |
| `trimCountBefore` / `trimCountAfter` | Conteo global. |
| `visibleDiagonalStitchesBefore` / `visibleDiagonalStitchesAfter` | Detector unificado. |
| `ce01StatusBefore` / `ce01StatusAfter` | Estado CE01. |
| `professionalScoreBefore` / `professionalScoreAfter` | Quality gate profesional. |
| `finalLookExportMismatchBefore` / `finalLookExportMismatchAfter` | Integridad visual/export. |
